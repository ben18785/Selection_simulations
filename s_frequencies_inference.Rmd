...
```{r, results=F}
colnames(aDF_short) <- c("generation", "variant", "count")
...
Run positive-frequency dependence Wright-Fisher with mutation
```{r}
selection <- 0.5
aDF_short <- Models$WrightFisherRun(as.integer(num_generations), as.integer(pop_size), mu, selection, as.integer(pop_size))
...
Run negative frequency dependence Wright-Fisher with mutation
```{r}
selection <- 0
aDF_short <- Models$WrightFisherRun(as.integer(num_generations), as.integer(pop_size), mu, selection, as.integer(pop_size))
...
```{r, results=F}
aDF_short <- read.csv("time_series_negative.csv")[, -1]
colnames(aDF_short) <- c("generation", "variant", "count")
data_stan <- fPrepareAll(aDF_short)
fit <- sampling(aModel,data=data_stan,iter=200,chains=4,
                init=initFn)
